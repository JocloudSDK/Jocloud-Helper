...
static const char* TAG = "CCrashStack";
...
std::string CCrashStack::GetModuleByRetAddr(PBYTE Ret_Addr, PBYTE& Module_Addr)
{
	...
	std::string sRet = ws2s(Module_Name);
	return sRet;
}

std::string CCrashStack::GetCallStack(PEXCEPTION_POINTERS pException)
{
	PBYTE   Module_Addr_1;
	char bufer[256] = { 0 };
	std::string sRet;
		...
		Ebp = Ebp->Ebp)
	{
		...
		std::string moduleName = this->GetModuleByRetAddr(Ebp->Ret_Addr, Module_Addr_1);
...
std::string CCrashStack::GetVersionStr()
{
	...
	std::string graphics_module = "GraphicsCard: ";
	std::string sound_module = "SoundDevice: ";

	GetHardwareInaformation(graphics_module, sound_module);
	std::string APP_VERSION = "12";
	std::string sRet = stringFormat("APP Version: %s\n", APP_VERSION.c_str());
	sRet.append(stringFormat("Windows:  %ld.%ld.%ld, SP %d.%d, Product Type %d\n", V.dwMajorVersion, V.dwMinorVersion, V.dwBuildNumber,
...
std::string CCrashStack::GetExceptionInfo()
{
	...
	std::string sTmp = GetVersionStr();
	sRet.append(sTmp);
	sRet.append("Process:  ");

	GetModuleFileName(NULL, Module_Name, MAX_PATH);
	sRet.append(ws2s(Module_Name));
	sRet.append("\n");
	...
	if (m_pException)
	{
		...
		std::string module = GetModuleByRetAddr((PBYTE)E.ExceptionAddress, Module_Addr);
		if (module.length() > 0)
		{
			sRet.append(" Module: ");
	...
	sRet.append("\nCall Stack:");
	std::string sCallstack = this->GetCallStack(m_pException);
	sRet.append(sCallstack);

	return sRet;
}

bool CCrashStack::GetHardwareInaformation(std::string& graphics_card, std::string& sound_deivce)
{
	HRESULT hres;
	...
	if (FAILED(hres))
	{
		Logw(TAG, Log().setMessage("Failed to initialize COM library. ").setMethod("CoInitializeEx"));
		return false;
	}
	...
	{
		Logw(TAG, Log().setMessage("Failed to initialize security. ").setMethod("CoInitializeSecurity"));
		CoUninitialize();
		return false;
	}
	...
	if (FAILED(hres))
	{
		Logw(TAG, Log().setMessage("Failed to create IWbemLocator object.").setMethod("CoCreateInstance"));
		CoUninitialize();
		return false;
	}
	...
	if (FAILED(hres))
	{
		Logw(TAG, Log().setMessage("Failed to ConnectServer.").setMethod("ConnectServer"));
		pLoc->Release();
		CoUninitialize();
		return false;
	}
	...
	if (FAILED(hres))
	{
		Logw(TAG, Log().setMessage("Could not set proxy blanket.").setMethod("CoSetProxyBlanket"));
		pSvc->Release();
		pLoc->Release();
		CoUninitialize();
		return false;
	...
	{
		Logw(TAG, Log().setMessage("Query for processes failed.").setMethod("ExecQuery"));
	...
	else
	{
		...
		while (pEnumerator_graphics)
		{
			...
			std::string description = ws2s(description_vtProp.bstrVal);

			hres_graphics = pclsObj_graphics->Get(L"DriverVersion", 0, &driver_version_vtProp, 0, 0);
			std::string driver_version = ws2s(driver_version_vtProp.bstrVal);

			hres_graphics = pclsObj_graphics->Get(L"Status", 0, &status_vtProp, 0, 0);
			std::string status = ws2s(status_vtProp.bstrVal);
