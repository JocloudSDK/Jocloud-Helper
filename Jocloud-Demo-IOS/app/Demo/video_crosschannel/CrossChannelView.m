...
{
    switch (type) {
        case ButtonTypeLocal: {
            self.playBtn.selected = selected;
        }
            break;
        case ButtonTypeRemote: {
            self.connectionBtn.selected = selected;
        }
            break;
        case ButtonTypeStream: {
            self.pushStreamBtn.selected = selected;
        }
            break;
        default:
            break;
...
{
    self.modulesManager.crossLocalUid = self.localUidView.inputText;
    self.modulesManager.crossLocalRoomId = self.localRoomidView.inputText;
    self.modulesManager.crossRemoteUid = self.remoteUidView.inputText;
    self.modulesManager.crossRemoteRoomId = self.remoteRoomidView.inputText;
    self.modulesManager.crossPushUrl = self.pushStreamView.inputText;
    if (self.localUidView.inputText.length == 0 || self.localRoomidView.inputText.length == 0) {
...
- (InputView *)createInputViewWithTitle:(NSString *)title placeholder:(NSString *)placeholder maxLength:(int)maxLength text:(NSString *)inputText;
{
...
- (UIButton *)createButtonWithNormalTitle:(NSString *)normalTitle selectTitle:(NSString *)selectTitle touchDownBg:(UIColor *)touchDownBg enableBg:(UIColor *)enableBg disEnableBg:(UIColor *)disEnableBg
{
    ...
    btn.jly_touchDownBgColor = touchDownBg;
    btn.jly_enableBgColor = enableBg;
    btn.jly_disEnableBgColor = disEnableBg;
    return btn;
}
...
- (InputView *)localUidView
{
    if (!_localUidView) {
        ...
        if (self.modulesManager.firstCreateData && self.modulesManager.crossLocalUid.length == 0) {
            inputText = [Utils generateRandomNumberWithDigitCount:6];
            self.modulesManager.crossLocalUid = inputText;
        } else {
            inputText = self.modulesManager.crossLocalUid;
        }
        _localUidView = [self createInputViewWithTitle:[NSBundle jly_localizedStringWithKey:@"本地UID"] placeholder:[NSBundle jly_localizedStringWithKey:@"最多6位数字"] maxLength:6 text:inputText];
    }
...
- (InputView *)localRoomidView
{
    ...
    if (self.modulesManager.firstCreateData && self.modulesManager.crossLocalRoomId.length == 0) {
        inputText = [Utils generateRandomNumberWithDigitCount:4];
        self.modulesManager.crossLocalRoomId = inputText;
    } else {
        inputText = self.modulesManager.crossLocalRoomId;
    }
    if (!_localRoomidView) {
        _localRoomidView = [self createInputViewWithTitle:[NSBundle jly_localizedStringWithKey:@"本地Room ID"] placeholder:[NSBundle jly_localizedStringWithKey:@"最多8位数字"] maxLength:8 text:inputText];
    }
...
{
    NSString *inputText = @"";
    if (self.modulesManager.crossRemoteUid.length > 0) {
        inputText = self.modulesManager.crossRemoteUid;
    }
    if (!_remoteUidView) {
        _remoteUidView = [self createInputViewWithTitle:[NSBundle jly_localizedStringWithKey:@"远端UID"] placeholder:[NSBundle jly_localizedStringWithKey:@"最多6位数字"] maxLength:6 text:inputText];
    }
...
{
    NSString *inputText = @"";
    if (self.modulesManager.crossRemoteRoomId.length > 0) {
        inputText = self.modulesManager.crossRemoteRoomId;
    }
    if (!_remoteRoomidView) {
        _remoteRoomidView = [self createInputViewWithTitle:[NSBundle jly_localizedStringWithKey:@"远端Room ID"] placeholder:[NSBundle jly_localizedStringWithKey:@"最多8位数字"] maxLength:8 text:inputText];
    }
...
{
    NSString *inputText = @"";
    if (self.modulesManager.crossPushUrl.length > 0) {
        inputText = self.modulesManager.crossPushUrl;
    }
    if (!_pushStreamView) {
        _pushStreamView = [self createInputViewWithTitle:[NSBundle jly_localizedStringWithKey:@"推流地址"] placeholder:@"rtmp://" maxLength:1000 text:inputText];
